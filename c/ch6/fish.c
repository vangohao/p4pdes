...
{
    PoissonCtx *user = (PoissonCtx *)ctx;
    return (-user->ca + (1.0 - user->ca) * 2.0 * x) * PetscExpReal(y + z); // note  f = - laplacian u = - 2 u
...
static DMDASNESFunctionFn *residual_ptr[3] = {(DMDASNESFunctionFn *)&Poisson1DFunctionLocal,
                                              (DMDASNESFunctionFn *)&Poisson2DFunctionLocal,
                                              (DMDASNESFunctionFn *)&Poisson3DFunctionLocal};

static DMDASNESJacobianFn *jacobian_ptr[3] = {(DMDASNESJacobianFn *)&Poisson1DJacobianLocal,
...
static PointwiseFcn g_bdry_ptr[3][3] = {{&u_exact_1Dmanupoly, &u_exact_1Dmanuexp, &zero},
                                        {&u_exact_2Dmanupoly, &u_exact_2Dmanuexp, &zero},
                                        {&u_exact_3Dmanupoly, &u_exact_3Dmanuexp, &zero}};

static PointwiseFcn f_rhs_ptr[3][3] = {{&f_rhs_1Dmanupoly, &f_rhs_1Dmanuexp, &zero},
...
int main(int argc, char **argv)
{
    ...
    PetscCall(PetscOptionsReal("-ca",
                               "set coefficient of advection term in equation",
                               "fish.c", user.ca, &user.ca, NULL));
